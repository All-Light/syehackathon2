...
export async function* kor(indata: Indata): AsyncGenerator<Handelse> {
  ...
  const egen = await profileraSjalv(indata.url);
  ...
  const hittade = await upptackKonkurrenter(egen, angivna, logg("upptack"));
  ...
  const arbete = parallellt(kandidater, 5, async ({ k, hittadAv }) => {
    const ut = await undersokKonkurrent(k, hittadAv, svensk, logg("undersok")).catch(
      () => null,
    );
    if (ut) klara.push(ut);
  ...
  const rapport = await syntetisera(egen, klara);
